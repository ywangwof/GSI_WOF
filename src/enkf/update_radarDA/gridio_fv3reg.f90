...
contains
  ...
  subroutine readgriddata(nanal1,nanal2,vars3d,vars2d,n3d,n2d,levels,ndim,ntimes,fileprefixes,reducedgrid,vargrid,qsat)
   ...
   real(r_double), dimension(npts,nlevs,ntimes,nanal2-nanal1+1), intent(out) :: qsat
    ...
    integer :: w_ind, ql_ind,qr_ind,qi_ind,qs_ins,qg_ind,dbz_ind
...
!    prse_ind = getindex(vars3d, 'prse') ! pressure
   ...
   if (oz_ind > 0) then
    ...
    allocate(workvar3d(nx_res,ny_res,nlevs))
       varstrname = 'ref_f3d'
       call read_fv3_restart_data3d(varstrname,fv3filename_phyvar,file_id_p,workvar3d)
    do k=1,nlevs
       nn = nn_tile0
       do j=1,ny_res
         do i=1,nx_res
            nn=nn+1
            vargrid(nn,levels(dbz_ind-1)+k,nb,ne)=workvar3d(i,j,k)
         enddo
       enddo
    enddo
...
subroutine writegriddata(nanal1,nanal2,vars3d,vars2d,n3d,n2d,levels,ndim,vargrid,no_inflate_flag)
    ...
    if (tv_ind > 0.or.tsen_ind>0 ) then
      ...
      enddo
       ...
       call write_fv3_restart_data3d(varstrname,fv3filename_tracer,file_id_t,qworkvar3d)
       do k=1,nlevs
          if (nproc .eq. 0)                                               &
    ...
    if (dbz_ind > 0) then
      ...
      do j=1,ny_res
         do i=1,nx_res
            nn=nn+1
            workinc3d(i,j,k)=vargrid(nn,levels(dbz_ind-1)+k,nb,ne)
         enddo
      enddo
      enddo
      workvar3d(1:nx_res,:,:)=workvar3d(1:nx_res,:,:)+workinc3d
      workvar3d(nx_res+1,:,:)=workvar3d(nx_res,:,:)
       call write_fv3_restart_data3d(varstrname,fv3filename_phyvar,file_id_p,workvar3d)
    ...
    call nc_check( nf90_close(file_id_t),&
      myname_,'close '//trim(filename_tracer) )

    call nc_check( nf90_close(file_id_p),&
      myname_,'close '//trim(filename_phyvar) )
