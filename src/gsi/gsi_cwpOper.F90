...
!                .      .    .                                       .
! subprogram:	 module gsi_cwpOper
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2018-08-10
!
! abstract: an obOper extension for cwpNode type
!
! program history log:
!   2018-08-10  j guo   - added this document block
!   2019-06-25  Hu      - add diag_radarcwp for controling radar reflectivity
!                         diag file (in module obsmod).
!   2019-07-22  j Guo   - moved diag_radarcwp and its description here from
!                         obsmod.
!
...
! module interface:
  ...
  implicit none
  public:: cwpOper      ! data stracture
  public:: diag_radarcwp

  type,extends(obOper):: cwpOper
  contains
...
!   def diag_radarcwp- namelist logical to compute/write (=true) radar
!                                          reflectiivty diag files
  logical,save:: diag_radarcwp=.false.

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  character(len=*),parameter :: myname='gsi_cwpOper'
  type(cwpNode),save,target:: myNodeMold_
...
contains
  ...
  subroutine setup_(self, lunin, mype, is, nobs, init_pass,last_pass)
    ...
    implicit none
    class(cwpOper ), intent(inout):: self
  ...
  subroutine intjo1_(self, ibin, rval,sval, qpred,sbias)
    !use intcwpmod, only: intjo => intcwp
  ...
  subroutine stpjo1_(self, ibin, dval,xval,pbcjo,sges,nstep,dbias,xbias)
    !use stpcwpmod, only: stpjo => stpcwp
    use gsi_bundlemod, only: gsi_bundle
    use bias_predictors, only: predictors
    use m_obsNode , only: obsNode
    use m_obsLList, only: obsLList_headNode
